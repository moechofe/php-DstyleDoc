 ...
 * Contient la classe de controle DstyleDoc, l'interface pour les converteurs DstyleDoc_Converter_Convert ainsi que diffÃ©rentes classes d'outils.
 */
 ...
 * Cette classe doit Ãªtre Ã©tendu.
 */
class DstyleDoc_Properties
{
  /**
   * Permet d'utiliser des getter.
   * __get() est appelÃ© automatiquement par PHP lorsque la lecture de donnÃ©e d'un membre est inaccessible.
   * __get() vÃ©rifiera au prÃ©alable que la fonction "get_"+nom_du_membre existe et quelle est appelable. Dans le cas contraire, une exception BadPropertyException sera lancÃ©.
   * Params:
   *   string $property = Le nom du membre.
   * Returns:
   *   mixed = Retournera la valeur retournÃ©e par la fonction "get_"+nom_du_membre.
   * Throws:
   *   BadPropertyException = LancÃ© si la fonction "get_"+nom_du_membre n'est pas disponible.
   */
...
class DstyleDoc_Element_Container
{
  ...
  /**
   * todo: optimiser : si l'object est dÃ©jÃ  en current($data) le retourner sans faire d'appel Ã  la base de donnÃ©e.
   */
