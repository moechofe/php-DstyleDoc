...
function d( $var = null )
{
	...
	else
	{
		...
		foreach( $trace as $entry )
			if( ! empty($entry['file']) and ! empty($entry['line']) )
			{
				$file = $entry['file'];
				$line = $entry['line'];
				break;
	...
	if( func_num_args() == 0 )
		return xdebug_front_end::add( new xdebug_front_null, $file, $line );
	else
		return xdebug_front_end::add( $var, $file, $line );
}
		...
		for( $i=0; $i< abs((integer)$var->pass); $i++ )
			$dump =
				preg_replace_callback(
					'/((?:	| {2})*(?:	| {2}))(<b>(?:object|array)<\/b>(?:\\(<i>(?:.*?)<\/i>\\)\\[<i>(?:\\d+)<\/i>\\])?)(\\r?\\n?)(\\r?\\n?(?:(?<!	| {2})\\1.*\\r?\\n?)*)/',
	...
	if( is_array($var->stack) ) :
		...
		foreach( $var->stack as $i => $stack )
		{
			...
			if( ! empty($stack['params']) )
			{
				...
				if( substr($dumpparams,0,39)=="<pre class='xdebug-var-dump' dir='ltr'>" ) $dumpparams = substr($dumpparams,39,-strlen('</pre>'));
				elseif( substr($dumpparams,0,15)=='<pre dir="ltr">' ) $dumpparams = substr($dumpparams,15,-strlen('</pre>'));
				else $dumpparams = substr($dumpparams,strlen('<pre>'),-strlen('</pre>'));
				$params = "<div class=\"params\" id=\"xdebug_stack_{$id}_{$i}\" style=\"display:none\">".nl2br($dumpparams).'<br></div>';
